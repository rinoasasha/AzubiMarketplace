<script>
    ...
    let username = '';
    let standort = '';
    let trainingStartYear = 1;
    let trainingOccupationAbbr = '';
...
async function postRequest() {
    ...
    if (isNaN(trainingStartYear) || trainingStartYear < 1) {
        postError = 'Lehrjahr must be a positive number.';
        postLoading = false;
        return;
    }
    ...
    const requestData = {
        ...
        surname: lastName,
        standort: standort,
        Jahrgang: Number(trainingStartYear),
        richtung: trainingOccupationAbbr,

    };

    try {
        const response = await fetch(`${API_BASE}/Register/azubi`, {
            method: 'POST',
            headers: {
...
<div class="post-form-container">
    ...
    <div class="form-group">
        <label for="trainingStartYear">Jahrgang:</label>
        <input type="number" id="trainingStartYear" bind:value={trainingStartYear} min="1" />
    </div>
    ...
    <div class="form-group">
        <label for="trainingOccupationAbbr">Richtung:</label>
        <input type="text" id="trainingOccupationAbbr" bind:value={trainingOccupationAbbr} />
    </div>
